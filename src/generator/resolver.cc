  ...
  if (result != this->schemas.cend()) {
    ...
    if (schema.is_object() && !schema.defines("$schema") &&
      ...
      schema.assign("$schema", sourcemeta::core::JSON{
                                   result->second.default_dialect.value()});
    }
    ...
    sourcemeta::core::reference_visit(
        ...
        result->second.reference_visitor, result->second.default_dialect,
...
auto Resolver::add(const Configuration &configuration,
  ...
  const auto result{this->schemas.emplace(
      ...
      Entry{
          canonical, collection.default_dialect, effective_identifier,
