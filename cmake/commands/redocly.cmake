...
function(sourcemeta_redocly_openapi_to_json)
  cmake_parse_arguments(SOURCEMETA_REDOCLY_OPENAPI_TO_JSON "" "REDOCLY;INPUT;OUTPUT" "DEPENDS" ${ARGN})

  if(NOT SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_REDOCLY)
    message(FATAL_ERROR "You must pass the REDOCLY option to ${CMAKE_CURRENT_FUNCTION}")
  endif()

  if(NOT SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_INPUT)
    message(FATAL_ERROR "You must pass the INPUT option to ${CMAKE_CURRENT_FUNCTION}")
  endif()

  if(NOT SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_OUTPUT)
    message(FATAL_ERROR "You must pass the OUTPUT option to ${CMAKE_CURRENT_FUNCTION}")
  endif()

  find_program(NODE_BIN NAMES node REQUIRED)

  set(SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_ALL_DEPS "${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_INPUT}")
  if(SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_DEPENDS)
    list(APPEND SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_ALL_DEPS ${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_DEPENDS})
  endif()

  add_custom_command(
    OUTPUT "${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_OUTPUT}"
    COMMAND "${NODE_BIN}" "${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_REDOCLY}"
      bundle --ext json --output "${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_OUTPUT}"
      "${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_INPUT}"
    DEPENDS ${SOURCEMETA_REDOCLY_OPENAPI_TO_JSON_ALL_DEPS})
endfunction()
